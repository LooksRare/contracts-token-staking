...
async function setupUsers(
  ...
  for (const user of users) {
    ...
    await looksRareToken
      .connect(user)
      .approve(aggregator.address, constants.MaxUint256);
...
describe("AggregatorFeeSharing", () => {
  ...
  beforeEach(async () => {
    ...
    const MockUniswapV3Router = await ethers.getContractFactory(
      "MockUniswapV3Router"
    );
    ...
    await looksRareToken
      .connect(admin)
      .transfer(uniswapRouter.address, parseEther("2250"));

    const AggregatorFeeSharingWithUniswapV3 = await ethers.getContractFactory(
  ...
  describe("#1 - Regular user/admin interactions", async () => {
    it("Users can use the contract as autocompounder if no reward", async () => {
      ...
      await setupUsers(feeSharingSystem, looksRareToken, aggregator, admin, [
        user1,
        user2,
        user3,
      ]);
      ...
      assert.deepEqual(
        await aggregator.userInfo(user1.address),
        parseEther("100")
      );
      assert.deepEqual(
        await aggregator.userInfo(user2.address),
        parseEther("100")
      );
      assert.deepEqual(
        await aggregator.userInfo(user3.address),
        parseEther("100")
      );
      ...
      assert.deepEqual(
        await feeSharingSystem.calculateSharesValueInLOOKS(aggregator.address),
        parseEther("300")
      );

      assert.deepEqual(
        await aggregator.calculateSharePriceInPrimeShare(),
        BigNumber.from((1e18).toString())
      );

      assert.deepEqual(
        await aggregator.calculateSharePriceInLOOKS(),
        BigNumber.from((1e18).toString())
      );
      ...
      await advanceBlockTo(
        BigNumber.from((await tokenDistributor.START_BLOCK()).toString()).add(
          constants.One
        )
      );
      ...
      assert.deepEqual(
        await aggregator.calculateSharesValueInLOOKS(user1.address),
        parseEther("110")
      );
      ...
      await expect(tx)
        .to.emit(aggregator, "Withdraw")
        .withArgs(user1.address, parseEther("120"));
    ...
    it("Users can deposit, it sells rewards automatically when above threshold", async () => {
      ...
      await rewardToken
        .connect(admin)
        .transfer(feeSharingSetter.address, parseEther("50"));
      ...
      expect(tx)
        ...
        .withArgs(
          parseEther("49.99999999999999980"),
          parseEther("99.9999999999999996")
        );
    });
    ...
    it("Users can deposit, it does not sell rewards if below threshold or if deactivated", async () => {
      ...
      await aggregator
        .connect(admin)
        .updateThresholdAmount(parseEther("50.000001"));
      await aggregator.connect(admin).startHarvest();
    ...
    it("Users can withdraw, it sells rewards automatically when above threshold", async () => {
      ...
      await advanceBlockTo(
        BigNumber.from((await tokenDistributor.START_BLOCK()).toString()).sub(
          "1"
        )
      );
      ...
      // Advanced to the end of the first fee-sharing
      await advanceBlockTo(
        BigNumber.from(await tokenDistributor.START_BLOCK()).add("49")
      );
      ...
      expect(tx)
        .to.emit(aggregator, "Withdraw")
        .withArgs(user1.address, parseEther("633.333333333333333198"));
      ...
      expect(tx)
        .to.emit(aggregator, "Withdraw")
        .withArgs(user2.address, parseEther("648.333333333133333194"));
    });
    ...
    it("Users can withdraw, it does not sell rewards if below threshold or if deactivated", async () => {
      ...
      assert.notEqual(
        await rewardToken.balanceOf(aggregator.address),
        constants.Zero
      );
    });
    ...
    it("Owner can harvest using the harvest functions", async () => {
      ...
      await rewardToken
        .connect(admin)
        .transfer(aggregator.address, parseEther("4.5"));
      ...
      expect(tx)
        .to.emit(aggregator, "ConversionToLOOKS")
        .withArgs(parseEther("4.5"), parseEther("9"));

      expect(tx)
        .to.emit(feeSharingSystem, "Deposit")
        .withArgs(aggregator.address, parseEther("9"));
    ...
    it("Harvest doesn't trigger reinvesting if amount received is less than 1 LOOKS", async () => {
      ...
      await aggregator
        .connect(admin)
        .updateThresholdAmount(parseEther("0.999"));
      await rewardToken
        .connect(admin)
        .transfer(aggregator.address, parseEther("0.999"));
      ...
      expect(tx)
        .to.emit(aggregator, "ConversionToLOOKS")
        .withArgs(parseEther("0.999"), parseEther("0.999"));
  ...
  describe("#2 - Revertions work as expected", async () => {
    it("Users cannot deposit if less than minimum or withdraw without funds", async () => {
      ...
      const minDepositAmount = BigNumber.from(
        (await aggregator.MINIMUM_DEPOSIT_LOOKS()).toString()
      );

      await expect(
        aggregator
          .connect(user1)
          .deposit(minDepositAmount.sub(BigNumber.from("1")))
      ).to.be.revertedWith("Deposit: Amount must be >= 1 LOOKS");

      await expect(
        aggregator.connect(noDepositUser).withdraw("0")
      ).to.be.revertedWith(
        "Withdraw: Shares equal to 0 or larger than user shares"
      );

      await expect(
        aggregator.connect(noDepositUser).withdraw("1")
      ).to.be.revertedWith(
        "Withdraw: Shares equal to 0 or larger than user shares"
      );

      await expect(
        aggregator.connect(noDepositUser).withdrawAll()
      ).to.be.revertedWith("Withdraw: Shares equal to 0");
    });
    ...
    it("Cannot deposit if it is paused", async () => {
      ...
      await expect(
        aggregator.connect(user1).deposit(parseEther("10"))
    ...
    it("Cannot harvest if no outstanding share", async () => {
      await expect(
        aggregator.connect(admin).harvestAndSellAndCompound()
      ).to.be.revertedWith("Harvest: No share");
    });

    it("Faulty router doesn't throw revertion on harvesting operations if it fails to sell", async () => {
      const MockFaultyUniswapV3Router = await ethers.getContractFactory(
        "MockFaultyUniswapV3Router"
      );
  ...
  describe("#3 - Admin functions", async () => {
    it("Owner can update UniswapV3 fee", async () => {
      const tx = await aggregator
        .connect(admin)
        .updateTradingFeeUniswapV3("10000");
      expect(tx)
        .to.emit(aggregator, "NewTradingFeeUniswapV3")
        .withArgs("10000");
      assert.equal(
        (await aggregator.tradingFeeUniswapV3()).toString(),
        "10000"
      );
    });
    ...
    it("Owner can update threshold", async () => {
      const tx = await aggregator
        .connect(admin)
        .updateThresholdAmount(parseEther("5"));
      expect(tx)
        .to.emit(aggregator, "NewThresholdAmount")
    ...
    it("Owner cannot pause/unpause if paused", async () => {
      await expect(aggregator.connect(admin).unpause()).to.be.revertedWith(
        "Pausable: not paused"
      );
      await aggregator.connect(admin).pause();
      await expect(aggregator.connect(admin).pause()).to.be.revertedWith(
        "Pausable: paused"
      );
    });
    ...
    it("Owner cannot update UniswapV3 fee if wrong fee", async () => {
      await expect(
        aggregator.connect(admin).updateTradingFeeUniswapV3("1")
      ).to.be.revertedWith("Owner: Fee invalid");
      await expect(
        aggregator.connect(admin).updateTradingFeeUniswapV3("9999")
      ).to.be.revertedWith("Owner: Fee invalid");
    });
    ...
    it("Only owner can call functions for onlyOwner", async () => {
      ...
      await expect(
        aggregator.connect(user1).harvestAndSellAndCompound()
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(
        aggregator.connect(user1).updateTradingFeeUniswapV3("200")
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(
        aggregator.connect(user1).updateThresholdAmount("10")
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(aggregator.connect(user1).pause()).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
      await expect(aggregator.connect(user1).unpause()).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
      await expect(aggregator.connect(user1).startHarvest()).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
      await expect(aggregator.connect(user1).stopHarvest()).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
      await expect(
        aggregator.connect(user1).checkAndAdjustLOOKSTokenAllowanceIfRequired()
      ).to.be.revertedWith("Ownable: caller is not the owner");
