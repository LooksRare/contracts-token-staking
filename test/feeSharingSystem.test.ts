...
describe("FeeSharingSystem", () => {
  ...
  beforeEach(async () => {
    ...
    [
      ...
      user3,
      user4,
      feeSharingRecipient1,
      feeSharingRecipient2,
      tokenSplitter,
    ...
    startBlock = BigNumber.from(await ethers.provider.getBlockNumber()).add(
      "100"
    );
    ...
    const FeeSharingSetter = await ethers.getContractFactory(
      "FeeSharingSetter"
    );
    ...
    await feeSharingSystem
      .connect(admin)
      .transferOwnership(feeSharingSetter.address);

    await feeSharingSetter
      .connect(admin)
      .grantRole(await feeSharingSetter.OPERATOR_ROLE(), admin.address);

    // Each user receives 500 LOOKS tokens and deposits 100 LOOKS
    for (const user of [user1, user2, user3, user4]) {
      await looksRareToken
        .connect(admin)
        .transfer(user.address, parseEther("200"));
      await looksRareToken
        .connect(user)
        .approve(feeSharingSystem.address, constants.MaxUint256);
  ...
  describe("#1 - Regular user/admin interactions", async () => {
    it("Users can use the contract as autocompounder if no reward", async () => {
      ...
      await advanceBlockTo(
        BigNumber.from(await tokenDistributor.START_BLOCK())
      );

      assert.deepEqual(
        await feeSharingSystem.calculateSharePriceInLOOKS(),
        BigNumber.from((1e18).toString())
      );

      // No reward
      assert.deepEqual(
        await feeSharingSystem.currentRewardPerBlock(),
        constants.Zero
      );

      assert.deepEqual(await feeSharingSystem.totalShares(), parseEther("400"));
      assert.deepEqual(
        await feeSharingSystem.lastRewardBlock(),
        await feeSharingSystem.periodEndBlock()
      );

      assert.deepEqual(
        await tokenDistributor.accTokenPerShare(),
        parseEther("0")
      );

      await advanceBlockTo(
        BigNumber.from(await tokenDistributor.START_BLOCK()).add(constants.One)
      );

      // 30 LOOKS per block / 4 = 7.5
      // New exchange rate is 107.5 for 100 LOOKS
      assert.deepEqual(
        await feeSharingSystem.calculateSharesValueInLOOKS(user1.address),
        parseEther("107.5")
      );
      ...
      let totalAmountStaked = (
        await tokenDistributor.userInfo(feeSharingSystem.address)
      )[0];

      let expectedAmountToReceive =
        await tokenDistributor.calculatePendingRewards(
          feeSharingSystem.address
        );
      ...
      await advanceBlockTo(
        BigNumber.from(await tokenDistributor.START_BLOCK()).add("3")
      );
      ...
      expectedAmountToReceive = await tokenDistributor.calculatePendingRewards(
        feeSharingSystem.address
      );
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "Withdraw")
        .withArgs(user2.address, expectedValue, parseEther("0"));
    ...
    it("Users can unstake and collect rewards", async () => {
      ...
      await rewardToken
        .connect(admin)
        .transfer(feeSharingSetter.address, parseEther("500"));
      ...
      await advanceBlockTo(
        BigNumber.from(await feeSharingSystem.periodEndBlock())
      );
    ...
    it("Users can deposit multiple times and harvest if requested", async () => {
      ...
      expect(tx)
        ...
        .withArgs("50", parseEther("10"), parseEther("500"));
      ...
      tx = await feeSharingSystem
        .connect(user1)
        .deposit(parseEther("10"), true);

      expect(tx).to.emit(tokenDistributor, "Deposit");

      expect(tx)
        .to.emit(tokenDistributor, "Deposit")
        .withArgs(user1.address, parseEther("10"), parseEther("125"));
    });
    ...
    it("Owner can adjust the block schedule", async () => {
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "NewRewardDurationInBlocks")
        .withArgs("100");

      assert.deepEqual(
        await feeSharingSetter.rewardDurationInBlocks(),
        BigNumber.from("50")
      );
      assert.deepEqual(
        await feeSharingSetter.nextRewardDurationInBlocks(),
        BigNumber.from("100")
      );
      ...
      await rewardToken
        .connect(admin)
        .transfer(feeSharingSetter.address, parseEther("800"));
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "NewRewardPeriod")
        .withArgs("100", parseEther("8"), parseEther("800"));
    ...
    it("Users can stake/unstake and collect WETH + LOOKS", async () => {
      ...
      assert.deepEqual(
        await feeSharingSystem.calculatePendingRewards(user1.address),
        parseEther("125")
      );
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "Withdraw")
        .withArgs(user1.address, parseEther("490"), parseEther("125"));
      ...
      await rewardToken
        .connect(admin)
        .transfer(feeSharingSetter.address, rewardAdded);
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "NewRewardPeriod")
        .withArgs("50", parseEther("10"), rewardAdded);
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "Harvest")
        .withArgs(user3.address, parseEther("2625"));
      ...
      const halfPeriodEndBlock = BigNumber.from(
        await feeSharingSystem.periodEndBlock()
      ).sub("26");
      ...
      expect(tx)
        ...
        .withArgs(
          user3.address,
          parseEther("1452.49999999956425"),
          parseEther("2500")
        );
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "Harvest")
        .withArgs(user4.address, parseEther("10125"));
      ...
      expect(tx)
        ...
        .withArgs(
          feeSharingSystem.address,
          parseEther("1857.499999997238228290")
        );
    ...
    it("Rewards are paid as expected for passive staking", async () => {
      ...
      const stakingAddresses = [
        feeSharingRecipient1.address,
        feeSharingRecipient2.address,
      ];

      let tx = await feeSharingSetter
        .connect(admin)
        .addFeeStakingAddresses(stakingAddresses);

      expect(tx)
        .to.emit(feeSharingSetter, "FeeStakingAddressesAdded")
        .withArgs(stakingAddresses);

      assert.includeOrderedMembers(
        await feeSharingSetter.viewFeeStakingAddresses(),
        stakingAddresses
      );

      await looksRareToken
        .connect(admin)
        .transfer(feeSharingRecipient1.address, parseEther("200"));

      await looksRareToken
        .connect(admin)
        .transfer(feeSharingRecipient2.address, parseEther("200"));
      ...
      expect(tx)
        .to.emit(feeSharingSystem, "NewRewardPeriod")
        .withArgs("50", parseEther("5"), parseEther("250"));
      ...
      tx = await feeSharingSetter
        .connect(admin)
        .removeFeeStakingAddresses([feeSharingRecipient1.address]);

      expect(tx)
        .to.emit(feeSharingSystem, "FeeStakingAddressesRemoved")
        .withArgs([feeSharingRecipient1.address]);
      ...
      assert.deepEqual(
        await rewardToken.balanceOf(feeSharingSystem.address),
        parseEther("625")
      );
    });
    ...
    it("Rewards can be converted with the reward convertor", async () => {
      ...
      await expect(
        feeSharingSetter.convertCurrencyToRewardToken(
          randomToken.address,
          defaultAbiCoder.encode([], [])
        )
      ).to.be.revertedWith("Convert: RewardConvertor not set");

      const MockRewardConvertor = await ethers.getContractFactory(
        "MockRewardConvertor"
      );
      const rewardConvertor = await MockRewardConvertor.deploy(
        feeSharingSetter.address
      );
      await rewardToken
        .connect(admin)
        .transfer(rewardConvertor.address, parseEther("10"));

      let tx = await feeSharingSetter
        .connect(admin)
        .setRewardConvertor(rewardConvertor.address);

      expect(tx)
        .to.emit(feeSharingSetter, "NewRewardConvertor")
        .withArgs(rewardConvertor.address);
      ...
      await randomToken
        .connect(admin)
        .mint(feeSharingSystem.address, parseEther("500"));

      await rewardToken
        .connect(admin)
        .transfer(rewardConvertor.address, parseEther("30"));
  ...
  describe("#2 - Alternative logic paths without FeeSharingSetter", async () => {
    it("Cannot update rewards if no reward", async () => {
      await feeSharingSetter
        .connect(admin)
        .transferOwnershipOfFeeSharingSystem(admin.address);

      // Transfer 500 WETH token to the contract (50 blocks with 10 WETH/block)
      await rewardToken
        .connect(admin)
        .transfer(feeSharingSystem.address, parseEther("500"));

      // Advance block rewards to startBlock - 1
      await advanceBlockTo(await tokenDistributor.START_BLOCK());

      // 1. Rewards are updated for the first phase
      let tx = await feeSharingSystem
        .connect(admin)
        .updateRewards(parseEther("500"), "50");

      assert.deepEqual(
        await feeSharingSystem.rewardPerTokenStored(),
        constants.Zero
      );
      ...
      await advanceBlockTo(
        BigNumber.from(await tokenDistributor.START_BLOCK()).add("24")
      );
      ...
      await advanceBlockTo(
        BigNumber.from(await feeSharingSystem.periodEndBlock()).sub("11")
      );

      tx = await feeSharingSystem
        .connect(admin)
        .updateRewards(parseEther("200"), "10");

      await rewardToken
        .connect(admin)
        .transfer(feeSharingSystem.address, parseEther("200"));
  ...
  describe("#3 - Revertions", async () => {
    it("Cannot update rewards if no reward", async () => {
      await expect(
        feeSharingSetter.connect(admin).updateRewards()
      ).to.be.revertedWith("Reward: Nothing to distribute");
    });
    ...
    it("Cannot update rewards if too early", async () => {
      ...
      await expect(
        feeSharingSetter.connect(admin).updateRewards()
      ).to.be.revertedWith("Reward: Too early to add");
    });
    ...
    it("Cannot deposit if amount is less than 1", async () => {
      await expect(
        feeSharingSystem
          .connect(user1)
          .deposit(parseEther("0.99999999999999"), true)
      ).to.be.revertedWith("Deposit: Amount must be >= 1 LOOKS");

      await expect(
        feeSharingSystem.connect(user1).deposit(parseEther("1").sub("1"), true)
      ).to.be.revertedWith("Deposit: Amount must be >= 1 LOOKS");
    });

    it("Cannot harvest if amount is 0", async () => {
      await expect(
        feeSharingSystem.connect(user1).harvest()
      ).to.be.revertedWith("Harvest: Pending rewards must be > 0");
    });

    it("Cannot withdraw if amount is 0 or larger than user balance", async () => {
      await expect(
        feeSharingSystem.connect(user1).withdraw("0", false)
      ).to.be.revertedWith(
      ...
      await expect(
        feeSharingSystem
          .connect(user1)
          .withdraw(parseEther("100.0000001"), false)
      ).to.be.revertedWith(
        "Withdraw: Shares equal to 0 or larger than user shares"
      );
    });
    ...
    it("Cannot add/remove if no/wrong address", async () => {
      await expect(
        feeSharingSetter
          .connect(admin)
          .removeFeeStakingAddresses([user1.address])
      ).to.be.revertedWith("Owner: Address not registered");

      await feeSharingSetter
        .connect(admin)
        .addFeeStakingAddresses([user1.address]);

      await expect(
        feeSharingSetter.connect(admin).addFeeStakingAddresses([user1.address])
      ).to.be.revertedWith("Owner: Address already registered");
    });

    it("Can set new reward duration only if within the range range", async () => {
      const minimumDuration =
        await feeSharingSetter.MIN_REWARD_DURATION_IN_BLOCKS();
      const maximumDuration =
        await feeSharingSetter.MAX_REWARD_DURATION_IN_BLOCKS();

      let tx = await feeSharingSetter
        .connect(admin)
        .setNewRewardDurationInBlocks(minimumDuration);

      expect(tx)
        .to.emit(feeSharingSetter, "NewRewardDurationInBlocks")
        .withArgs(minimumDuration);

      tx = await feeSharingSetter
        .connect(admin)
        .setNewRewardDurationInBlocks(maximumDuration);

      expect(tx)
        .to.emit(feeSharingSetter, "NewRewardDurationInBlocks")
        .withArgs(maximumDuration);

      await expect(
        feeSharingSetter
          .connect(admin)
          .setNewRewardDurationInBlocks(
            BigNumber.from(minimumDuration).sub(BigNumber.from("1"))
          )
      ).to.be.revertedWith(
      ...
      await expect(
        feeSharingSetter
          .connect(admin)
          .setNewRewardDurationInBlocks(
            BigNumber.from(maximumDuration).add(BigNumber.from("1"))
          )
      ).to.be.revertedWith(
        "Owner: New reward duration in blocks outside of range"
      );
  ...
  describe("#3 - Owner", async () => {
    it("Can transfer ownership of FeeSharingSystem", async () => {
      ...
      await expect(
        feeSharingSetter
          .connect(admin)
          .transferOwnershipOfFeeSharingSystem(constants.AddressZero)
      ).to.be.revertedWith("Owner: New owner cannot be null address");

      const tx = await feeSharingSetter
        .connect(admin)
        .transferOwnershipOfFeeSharingSystem(admin.address);

      expect(tx)
        .to.emit(feeSharingSetter, "NewFeeSharingSystemOwner")
    ...
    it("Owner functions can only be called by owner", async () => {
      ...
      await expect(
        feeSharingSetter.connect(user1).updateRewards()
      ...
      await expect(
        feeSharingSetter.connect(user1).setNewRewardDurationInBlocks("300")
      ).to.be.revertedWith(ERROR_MESSAGE_DEFAULT_ADMIN_ROLE);

      await expect(
        feeSharingSetter
          .connect(user1)
          .setRewardConvertor(constants.AddressZero)
      ).to.be.revertedWith(ERROR_MESSAGE_DEFAULT_ADMIN_ROLE);

      await expect(
        feeSharingSetter.connect(user1).addFeeStakingAddresses([user1.address])
      ).to.be.revertedWith(ERROR_MESSAGE_DEFAULT_ADMIN_ROLE);
      ...
      await expect(
        feeSharingSetter
          .connect(user1)
          .transferOwnershipOfFeeSharingSystem(user1.address)
      ).to.be.revertedWith(ERROR_MESSAGE_DEFAULT_ADMIN_ROLE);
    });
  });
});
